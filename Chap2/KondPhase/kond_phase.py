...
rho_peak = 0.29*2.0e12/1e-6 # peak density in cm^-3/centi^-3
...
#    print(intersection)
    intersection=-d_c
    chi_nb = susceptibility(intersection+0*2*np.pi*0.57*10**6/Gamma_e, d_c, gamma_21, om_c)
    phi_0=omega_s/(2*c) * L * chi_0 * np.real(chi_nb)
    
    r_chi_vdw = lambda x: np.real(susceptibility(intersection+0*2*np.pi*0.57*10**6/Gamma_e, d_c - vdW_pot(x, 1e-10)/(hbar*Gamma_e), gamma_21, om_c))
...
plot_dict['121']={
    'A':{'type':'plot','y':h[0].to_json(),'ylabel':u'$\Delta \\varphi$ (rad)','xlabel':u'$\Delta_s/ \Gamma_3$','margin':(0.1,0.2),'num':'a','xlim':(-4.2,-1.6),'ylim':(1,4.8),'label':'Num. Integration'},                
